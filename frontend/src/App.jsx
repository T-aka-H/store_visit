...
const processUploadedAudio = async () => {
  ...
  try {
    ...
    if (!recognitionRef.current) {
      throw new Error('音声認識機能が利用できません。ブラウザが対応していない可能性があります。');
    }
    ...
    recognition.onresult = (event) => {
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if (event.results[i].isFinal) {
          ...
          interimTranscript += transcript;import React, { useState, useEffect, useRef } from 'react';
...
const processUploadedAudio = async () => {
  ...
  try {
    ...
    formData.append('source', 'file_upload'); // リアルタイム認識と区別

    console.log('Gemini APIリクエスト送信中...');

    // Gemini 1.5 Flash音声認識専用エンドポイント
...
右下の青いマイクボタンでリアルタイム音声認識をご利用ください。`;
      ...
      userMessage = 'Gemini APIサーバーエラーが発生しました。しばらく時間をおいてから再試行してください。';
    }
