...
// Speech-to-Text用の音声形式変換
function getMimeTypeForSpeechAPI(originalMimeType) {
  const mimeMapping = {
  ...
  return mimeMapping[originalMimeType] || 'LINEAR16';
}
...
app.post('/api/transcribe', upload.single('audio'), async (req, res) => {
  try {
    ...
    // Speech-to-Text API設定確認
    if (!process.env.GOOGLE_APPLICATION_CREDENTIALS && !process.env.GOOGLE_CLOUD_PROJECT_ID) {
      console.error('Google Cloud認証情報が設定されていません');
      return res.status(500).json({ 
        error: 'Speech-to-Text API設定エラー',
        transcript: 'Speech-to-Text API設定に問題があります',
        categorized_items: []
      });
    }
    ...
    if (mimeType === 'audio/mp4' || mimeType === 'audio/m4a' || mimeType === 'audio/x-m4a') {
      console.log('iPhone音声ファイルを検出');
      mimeType = 'audio/mp4';
    }

    // Speech-to-Text APIリクエスト設定
    const audioConfig = {
      encoding: getMimeTypeForSpeechAPI(mimeType),
      sampleRateHertz: 16000, // iPhoneの録音設定に合わせる
      languageCode: 'ja-JP',
    ...
    const audio = {
      content: bufferToBase64(audioBuffer),
    };
    ...
    console.log('Speech-to-Text APIリクエスト送信中...');
    console.log('音声設定:', audioConfig);
    ...
    try {
      ...
      if (response.results && response.results.length > 0) {
        ...
        console.log('音声認識成功:', transcript);
      } else {
        transcript = '音声を認識できませんでした';
        console.log('音声認識結果なし');
      }
      ...
      console.log('カテゴリ自動分類開始');
      Object.entries(keywords).forEach(([category, keywordList]) => {
        const matchedKeywords = keywordList.filter(keyword => transcript.includes(keyword));
      ...
      const finalResult = {
        ...
        categorized_items: uniqueItems
      };

      console.log('=== Speech-to-Text 最終レスポンス ===');
      ...
      console.error('Speech-to-Text API エラー:', speechError);
      ...
      try {
        ...
        res.json({
          transcript: content.trim() || 'バックアップ音声認識に失敗しました',
          categorized_items: [],
          fallback: 'gemini'
        });

      } catch (geminiError) {
        console.error('Geminiバックアップも失敗:', geminiError);
        
        res.status(500).json({
          error: '音声認識エラー',
          transcript: `音声認識に失敗しました: ${speechError.message}`,
          categorized_items: []
        });
      }
    }
